package tr.com.hive.smm.codecs.time;

import org.bson.BsonReader;
import org.bson.BsonWriter;
import org.bson.codecs.Codec;
import org.bson.codecs.DecoderContext;
import org.bson.codecs.EncoderContext;

import java.time.DayOfWeek;

import static java.util.Objects.requireNonNull;

public class DayOfWeekAsInt32Codec implements Codec<DayOfWeek> {

  @Override
  public void encode(BsonWriter writer, DayOfWeek value, EncoderContext encoderContext) {
    requireNonNull(writer, "writer is null");
    requireNonNull(value, "value is null");

    writer.writeInt32(value.getValue());
  }

  @Override
  public DayOfWeek decode(BsonReader reader, DecoderContext decoderContext) {
    requireNonNull(reader, "reader is null");

    int dayOfWeekInt = reader.readInt32();

    return DayOfWeek.of(dayOfWeekInt);
  }

  @Override
  public Class<DayOfWeek> getEncoderClass() {
    return DayOfWeek.class;
  }

}
